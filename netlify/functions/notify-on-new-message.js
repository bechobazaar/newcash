...
exports.handler = async (event) => {
  ...
  try {
    ...
    if (arr.length){
      ...
      const { title, body } = buildSafeTitleBodyForChat(); // privacy

      // DATA-ONLY (no notification object) => Chrome auto-card off
      const msg = {
        tokens: arr,
        data: {
          title, body, url: link, tag,
          ch: 'fcm',
          kind: 'chat',                     // SW will enforce empty body
          chatId, senderId: String(m.senderId || ''), messageId: String(messageId)
        },
