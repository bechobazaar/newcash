...
const pickOrigin = (e)=>{ const h=e.headers||{}; const o=h.origin||h.Origin|| (h.host?`https://${h.host}`:''); return ALLOWED_ORIGINS.includes(o)?o:(process.env.APP_BASE_URL||ALLOWED_ORIGINS[0]||'').replace(/\/$/,''); };
...
exports.handler = async (event) => {
  ...
  try {
    ...
    let payload;
    try { payload = JSON.parse(event.body || '{}'); } catch { return { statusCode: 400, headers: cors, body: 'Invalid JSON body' }; }
    ...
    console.log('chatId', chatId, 'messageId', messageId, 'sender', m.senderId, 'users', users, 'recipients', recipients);
    ...
    console.log('tokens found:', tokenDocs.length);
    ...
    for (let i = 0; i < tokenDocs.length; i += 500) {
      ...
      res.responses.forEach((r, idx) => {
        details.push({ ok: r.success, code: r.error?.code || null, msg: r.error?.message || null, tokenIdx: i+idx });
        if (!r.success) {
    ...
    console.log('send summary:', { success, failed });

    return { statusCode: 200, headers: cors, body: JSON.stringify({ sent: success, failed, recipients, tokens: tokenDocs.length, details }) };
  } catch (err) {
