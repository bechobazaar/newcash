...
exports.handler = async (ev) => {
  if (ev.httpMethod === "OPTIONS") return ok({});         // CORS preflight
  if (ev.httpMethod !== "POST")    return bad(405, "Use POST");

  const key = ev.headers["x-gemini-key"] || process.env.GEMINI_API_KEY;
  ...
  const pct = (p) => {
    ...
    const t = idx - lo; return prices[lo]*(1-t) + prices[hi]*t;
  };
  ...
  const reqBody = {
    contents: [{
      ...
      parts: [
        { text: sys },
        { text: "ITEM JSON:" },
        { text: JSON.stringify({ item, priceBands, comps: topComps }, null, 2) },
        { text: `
