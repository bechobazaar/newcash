// (A2) Direct push to a single Appilix user_identity
exports.handler = async (event) => {
  try {
    ...
    const { title, body, user_identity, open_link_url } = JSON.parse(event.body || "{}");
    if (!title || !body || !user_identity) {
      return { statusCode: 400, body: "title, body, user_identity required" };
    }
    ...
    form.set("user_identity", user_identity); // <-- Target one user
